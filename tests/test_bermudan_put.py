...
def test_bermudan_put_fine():
    # 1. Initialize the input configuration values
    config = OptionConfig(
        deriv=DerivType.BermudanPut,
...
def test_bermudan_put_matches_binomial_across_maturities(maturity):
    ...
    1y but mispriced by ~$1-2 at every other maturity. This sweeps maturities — including the
    previously-broken short (0.25, 0.5) and long (2.0) cases plus a non-quarter-aligned one
    (0.6) — and requires the FDM to track the binomial's calendar-time exercise schedule.
    """
    config = OptionConfig(
        deriv=DerivType.BermudanPut,
        frequency=FrequencyType.Quarterly,
